...
| 2026-09-17 | Claude | **跟进 spring 变更**：`import_daily` 的 `-p` 由打屏改为按股票导出 CSV 到 `csv/`，且**必须同时带 `-c`**（约束在 `parse_arguments()` 里，`build_parser()` 保持无副作用供自省，故只由 help 承载——契约 C4 的典型场景）；同步改写 `etl_import_daily` 的 Tool 说明与 USAGE / INSTALL 中英的干跑描述（原文在建议模型做一件必然失败的事），重生成快照，修正干跑用例「零副作用」的注释。296 passed，integration 13 passed |
